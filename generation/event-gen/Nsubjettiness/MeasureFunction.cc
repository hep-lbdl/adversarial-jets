...
FASTJET_BEGIN_NAMESPACE // defined in fastjet/internal/base.hh
    ...
    // Return all of the necessary TauComponents for specific input particles
    // and axes
    TauComponents MeasureFunction::result(
        ...
        std::vector<fastjet::PseudoJet> jetPartitionStorage =
            get_partition(particles, axes, &beamPartitionStorage);

        // then return result calculated from partition
        return result_from_partition(jetPartitionStorage, axes,
                                     &beamPartitionStorage);
    }

    std::vector<fastjet::PseudoJet> MeasureFunction::get_partition(
        const std::vector<fastjet::PseudoJet> &particles,
        ...
        // Figures out the partiting of the input particles into the various jet
        // pieces
        ...
        for (unsigned i = 0; i < particles.size(); i++) {
            ...
            double minRsq;
            if (_has_beam)
                minRsq = beam_distance_squared(particles[i]);
            else
                minRsq =
                    std::numeric_limits<double>::max(); // make it large value

            // check to see which axis the particle is closest to
            for (unsigned j = 0; j < axes.size(); j++) {
                double tempRsq = jet_distance_squared(
                    particles[i], axes[j]); // delta R distance
                if (tempRsq < minRsq) {
                    minRsq = tempRsq;
                    j_min = j;
                }
            ...
            if (j_min == -1) {
                if (_has_beam)
                    beamPartition.push_back(particles[i]);
                else
                    assert(_has_beam); // this should never happen.
            } else {
        ...
        std::vector<PseudoJet> jetPartitionStorage(axes.size(),
                                                   PseudoJet(0, 0, 0, 0));
        for (unsigned j = 0; j < axes.size(); j++) {
    ...
    std::vector<std::list<int>> MeasureFunction::get_partition_list(
        const std::vector<fastjet::PseudoJet> &particles,
        const std::vector<fastjet::PseudoJet> &axes) const {
    ...
    TauComponents MeasureFunction::result_from_partition(
        const std::vector<fastjet::PseudoJet> &jet_partition,
        const std::vector<fastjet::PseudoJet> &axes,
        PseudoJet *beamPartitionStorage) const {
        ...
        if (!_has_denominator)
            tauDen =
                1.0; // if no denominator, then 1.0 for no normalization factor

        // first find jet pieces
        for (unsigned j = 0; j < axes.size(); j++) {
            std::vector<PseudoJet> thisPartition =
                jet_partition[j].constituents();
            for (unsigned i = 0; i < thisPartition.size(); i++) {
                jetPieces[j] += jet_numerator(thisPartition[i],
                                              axes[j]); // numerator jet piece
                if (_has_denominator)
                    tauDen += denominator(thisPartition[i]); // denominator
            }
        }
        ...
        if (_has_beam) {
            ...
            for (unsigned i = 0; i < beamPartition.size(); i++) {
                beamPiece +=
                    beam_numerator(beamPartition[i]); // numerator beam piece
                if (_has_denominator)
                    tauDen += denominator(beamPartition[i]); // denominator
            }
        }
        return TauComponents(jetPieces, beamPiece, tauDen, _has_denominator,
                             _has_beam);
    }
