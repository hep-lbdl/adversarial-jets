...
FASTJET_BEGIN_NAMESPACE // defined in fastjet/internal/base.hh
    ...
    //------------------------------------------------------------------------
    /// \class AxesFinder
    // This is the base class for all axes finders. These axes are used along
    // with the MeasureFunctions to calculate
    // tau_N. There are different implementations of axes finding that are
    // defined in derived classes below.
    class AxesFinder {

      public:
        // This function should be overloaded, and updates the seedAxes to
        // return new axes
        virtual std::vector<fastjet::PseudoJet>
        getAxes(int n_jets, const std::vector<fastjet::PseudoJet> &inputs,
    ...
    // This class finds axes by clustering the particles and then finding the
    // exclusive jets. This can be implemented
    ...
    // This class finds axes by finding the exlusive jets after clustering
    // according to a kT algorithm and a
      ...
      public:
        AxesFinderFromWTA_KT()
            : AxesFinderFromExclusiveJetDefinition(fastjet::JetDefinition(
                  ...
                  fastjet::JetDefinition::max_allowable_R, // maximum jet radius
                                                           // constant
                  &_recomb, fastjet::Best)) {
        }
    ...
    // This class finds axes by finding the exlusive jets after clustering
    // according to a CA algorithm and a
    // winner take all recombination scheme.
    ...
    // This class finds axes by clustering the particles and then finding the n
    // hardest inclusive jets.
      ...
      public:
        ...
        getAxes(int n_jets, const std::vector<fastjet::PseudoJet> &inputs,
            ...
            std::vector<fastjet::PseudoJet> myJets =
                sorted_by_pt(jet_clust_seq.inclusive_jets());
    ...
    // This class finds axes by finding the n hardest jets after clustering the
    // particles according
      ...
      public:
        AxesFinderFromAntiKT(double R0)
            : AxesFinderFromHardestJetDefinition(
                  fastjet::JetDefinition(fastjet::antikt_algorithm, R0,
                                         fastjet::E_scheme, fastjet::Best)) {
        }
    };
    ...
    // This class defines an AxesFinder that uses Kmeans minimization, but only
    // on a single pass.
    class AxesFinderFromOnePassMinimization : public AxesFinder {
      ...
      public:
        ...
        AxesFinderFromOnePassMinimization(double beta, double Rcutoff)
            : _precision(0.0001), // hard coded for now
              ...
              _beta(beta), _Rcutoff(Rcutoff), _measureFunction(beta, Rcutoff) {
        }

        virtual std::vector<fastjet::PseudoJet>
        getAxes(int n_jets, const std::vector<fastjet::PseudoJet> &inputJets,
                const std::vector<fastjet::PseudoJet> &currentAxes) const;
      ...
      private:
        ...
        UpdateAxesFast(const std::vector<LightLikeAxis> &old_axes,
                       const std::vector<fastjet::PseudoJet> &inputJets) const;

        std::vector<LightLikeAxis>
        UpdateAxes(const std::vector<LightLikeAxis> &old_axes,
                   const std::vector<fastjet::PseudoJet> &inputJets) const;
    };

    //------------------------------------------------------------------------
    /// \class AxesFinderFromKmeansMinimization
    // This class finds finds axes by using Kmeans clustering to minimizaiton
    // N-jettiness. Given a first set of
    // starting axes, it updates n times to get as close to the global minimum
    // as possible. This class calls OnePass many times,
    // added noise to the axes.
    class AxesFinderFromKmeansMinimization : public AxesFinder {

      public:
        AxesFinderFromKmeansMinimization(double beta, double Rcutoff,
                                         int n_iterations)
            : _n_iterations(n_iterations),
              _noise_range(1.0), // hard coded for the time being
              _measureFunction(beta, Rcutoff), _onePassFinder(beta, Rcutoff) {
        }
      ...
      private:
        int _n_iterations;   // Number of iterations to run  (0 for no
                             // minimization, 1 for one-pass, >>1 for global
                             // minimum)
        double _noise_range; // noise range for random initialization

        DefaultUnnormalizedMeasureFunction
            _measureFunction; // function to test whether minimum is reached

        AxesFinderFromOnePassMinimization
            _onePassFinder; // one pass finder that is repeatedly called
    ...
    // This class finds axes by minimizing the Lorentz dot product distance
    // between axes and particles. Given a first set of starting axes,
    ...
    class AxesFinderFromGeometricMinimization : public AxesFinder {
      ...
      public:
        AxesFinderFromGeometricMinimization(double beta, double Rcutoff)
            : _nAttempts(100), _accuracy(0.000000001),
              _function(beta, Rcutoff) {
            if (beta != 2.0) {
                throw Error("Geometric minimization is currently only defined "
                            "for beta = 2.0.");
            }
        }

        virtual std::vector<fastjet::PseudoJet>
        getAxes(int n_jets, const std::vector<fastjet::PseudoJet> &particles,
                const std::vector<fastjet::PseudoJet> &currentAxes) const;
    ...
    //------------------------------------------------------------------------
    /// \class LightLikeAxis
    // This is a helper class for the minimum Axes Finders classes above. It
    // creates a convenient way of defining axes
    ...
    class LightLikeAxis {
      ...
      public:
        ...
        LightLikeAxis(double my_rap, double my_phi, double my_weight,
                      double my_mom)
        ...
        void set_weight(double my_set_weight) {
            _weight = my_set_weight;
        }
        void set_mom(double my_set_mom) {
            _mom = my_set_mom;
        }
        void reset(double my_rap, double my_phi, double my_weight,
                   double my_mom) {
            _rap = my_rap;
            _phi = my_phi;
            _weight = my_weight;
