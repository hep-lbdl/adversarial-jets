...
FASTJET_BEGIN_NAMESPACE // defined in fastjet/internal/base.hh
    ...
    // This class contains the same information as Njettiness, but redoes it in
    // terms of the ClusterSequence::Extras class.
    // This is done in order to help improve the interface for the main
    // NjettinessPlugin class.
    // TODO:  This class should probably be merged with TauComponents, since
    // both have access
    ...
    class NjettinessExtras : public ClusterSequence::Extras {
      ...
      public:
        NjettinessExtras(TauComponents tau_components,
            ...
            : _tau_components(tau_components), _jets(jets), _axes(axes) {
        }
        ...
        std::vector<double> subTaus() const {
            return _tau_components.jet_pieces();
        }
        std::vector<fastjet::PseudoJet> jets() const {
            return _jets;
        }
        std::vector<fastjet::PseudoJet> axes() const {
            return _axes;
        }
        ...
        double subTau(const fastjet::PseudoJet &jet) const {
            if (labelOf(jet) == -1)
                return std::numeric_limits<double>::quiet_NaN(); // nonsense
    ...
    inline const NjettinessExtras *njettiness_extras(
        const fastjet::PseudoJet &jet) {
        const ClusterSequence *myCS = jet.associated_cluster_sequence();
        if (myCS == NULL)
            return NULL;
        const NjettinessExtras *extras =
            dynamic_cast<const NjettinessExtras *>(myCS->extras());
        return extras;
    }

    inline const NjettinessExtras *njettiness_extras(
        const fastjet::ClusterSequence &myCS) {
        const NjettinessExtras *extras =
            dynamic_cast<const NjettinessExtras *>(myCS.extras());
        return extras;
    }
    ...
    /**
     * An exclusive jet finder that identifies N jets; first N axes are found,
     * then
     ...
     *
     *   DR_ij is the distance sqrt(Delta_phi^2 + Delta_rap^2) between particle
     * i
     *   and jet j.
     *
     * The NormalizedMeausure include an extra parameter R0, and the various
     * cutoff
    ...
    class NjettinessPlugin : public JetDefinition::Plugin {
      public:
        ...
        NjettinessPlugin(int N, const AxesDefinition &axes_def,
        ...
        NjettinessPlugin(int N, Njettiness::AxesMode axes_mode,
        ...
        NjettinessPlugin(int N, Njettiness::AxesMode mode, double beta,
                         double R0,
                         double Rcutoff = std::numeric_limits<double>::max())
            : _njettinessFinder(mode,
