...
FASTJET_BEGIN_NAMESPACE // defined in fastjet/internal/base.hh
    ...
    // Given starting axes, update to find better axes by using Kmeans
    // clustering around the old axes
    template <int N>
    std::vector<LightLikeAxis>
    AxesFinderFromOnePassMinimization::UpdateAxesFast(
        const std::vector<LightLikeAxis> &old_axes,
        ...
        for (unsigned i = 0; i < inputJets.size(); i++) {
            double smallestDist =
                std::numeric_limits<double>::max(); // large number
            for (int k = 0; k < N; k++) {
        ...
        for (unsigned i = 0; i < inputJets.size(); i++) {
            ...
            // optimize pow() call
            // add noise (the precision term) to make sure we don't divide by
            // zero
            if (_beta == 1.0) {
                double DR = std::sqrt(
                    sq(precision) + old_axes[old_jet_i].DistanceSq(inputJet_i));
                old_dist = 1.0 / DR;
            } else if (_beta == 2.0) {
                old_dist = 1.0;
            } else if (_beta == 0.0) {
            ...
            // rapidity sum
            new_axis_i.set_rap(new_axis_i.rap() +
                               inputJet_i.perp() * inputRap_i * old_dist);
            // phi sum
            distPhi = inputPhi_i - old_axes[old_jet_i].phi();
            if (fabs(distPhi) <= M_PI) {
                new_axis_i.set_phi(new_axis_i.phi() +
                                   inputJet_i.perp() * inputPhi_i * old_dist);
            } else if (distPhi > M_PI) {
                new_axis_i.set_phi(new_axis_i.phi() +
                                   inputJet_i.perp() *
                                       (-2 * M_PI + inputPhi_i) * old_dist);
            } else if (distPhi < -M_PI) {
                new_axis_i.set_phi(new_axis_i.phi() +
                                   inputJet_i.perp() *
                                       (+2 * M_PI + inputPhi_i) * old_dist);
            ...
            // weights sum
            new_axis_i.set_weight(new_axis_i.weight() +
                                  inputJet_i.perp() * old_dist);
        ...
        for (int k = 0; k < N; k++) {
            if (new_axes[k].weight() == 0) {
                // no particles were closest to this axis!  Return to old axis
                // instead of (0,0,0,0)
                ...
                new_axes[k].set_phi(
                    std::fmod(new_axes[k].phi() + 2 * M_PI, 2 * M_PI));
                new_axes[k].set_mom(std::sqrt(new_jets[k].modp2()));
            }
        }
        std::vector<LightLikeAxis> new_axes_vec(N);
        for (unsigned k = 0; k < N; ++k)
            new_axes_vec[k] = new_axes[k];
        return new_axes_vec;
    }

    // Given N starting axes, this function updates all axes to find N better
    // axes.
    // (This is just a wrapper for the templated version above.)
    std::vector<LightLikeAxis> AxesFinderFromOnePassMinimization::UpdateAxes(
        const std::vector<LightLikeAxis> &old_axes,
        const std::vector<fastjet::PseudoJet> &inputJets) const {
        int N = old_axes.size();
        switch (N) {
        case 1:
            return UpdateAxesFast<1>(old_axes, inputJets);
        case 2:
            return UpdateAxesFast<2>(old_axes, inputJets);
        case 3:
            return UpdateAxesFast<3>(old_axes, inputJets);
        case 4:
            return UpdateAxesFast<4>(old_axes, inputJets);
        case 5:
            return UpdateAxesFast<5>(old_axes, inputJets);
        case 6:
            return UpdateAxesFast<6>(old_axes, inputJets);
        case 7:
            return UpdateAxesFast<7>(old_axes, inputJets);
        case 8:
            return UpdateAxesFast<8>(old_axes, inputJets);
        case 9:
            return UpdateAxesFast<9>(old_axes, inputJets);
        case 10:
            return UpdateAxesFast<10>(old_axes, inputJets);
        case 11:
            return UpdateAxesFast<11>(old_axes, inputJets);
        case 12:
            return UpdateAxesFast<12>(old_axes, inputJets);
        case 13:
            return UpdateAxesFast<13>(old_axes, inputJets);
        case 14:
            return UpdateAxesFast<14>(old_axes, inputJets);
        case 15:
            return UpdateAxesFast<15>(old_axes, inputJets);
        case 16:
            return UpdateAxesFast<16>(old_axes, inputJets);
        case 17:
            return UpdateAxesFast<17>(old_axes, inputJets);
        case 18:
            return UpdateAxesFast<18>(old_axes, inputJets);
        case 19:
            return UpdateAxesFast<19>(old_axes, inputJets);
        case 20:
            return UpdateAxesFast<20>(old_axes, inputJets);
        default:
            std::cout
    ...
    // uses minimization of N-jettiness to continually update axes until
    // convergence.
    // The function returns the axes found at the (local) minimum
    std::vector<fastjet::PseudoJet> AxesFinderFromOnePassMinimization::getAxes(
        int n_jets, const std::vector<fastjet::PseudoJet> &inputJets,
        ...
        int h = 0;
        while (cmp > _precision && h < _halt) { // Keep updating axes until
                                                // near-convergence or too many
                                                // update steps
            cmp = 0.0;
            h++;
        ...
        // this is used to debug the minimization routine to make sure that it
        // works.
        bool do_debug = false;
        if (do_debug) {
            // get this information to make sure that minimization is working
            // properly
            TauComponents seed_tau_components =
                _measureFunction.result(inputJets, seedAxes);
            double seed_tau = seed_tau_components.tau();
            TauComponents tau_components =
                _measureFunction.result(inputJets, outputAxes);
    ...
    PseudoJet AxesFinderFromKmeansMinimization::jiggle(const PseudoJet &axis)
        const {
        double phi_noise =
            ((double)rand() / (double)RAND_MAX) * _noise_range * 2.0 -
            _noise_range;
        double rap_noise =
            ((double)rand() / (double)RAND_MAX) * _noise_range * 2.0 -
            _noise_range;
    ...
    std::vector<fastjet::PseudoJet> AxesFinderFromKmeansMinimization::getAxes(
        int n_jets, const std::vector<fastjet::PseudoJet> &inputJets,
        const std::vector<fastjet::PseudoJet> &seedAxes) const {

        // first iteration
        std::vector<fastjet::PseudoJet> bestAxes =
            _onePassFinder.getAxes(n_jets, inputJets, seedAxes);

        double bestTau = (_measureFunction.result(inputJets, bestAxes)).tau();

        for (int l = 1; l < _n_iterations;
             l++) { // Do minimization procedure multiple times (l = 1 to start
                    // since first iteration is done already)
            ...
            std::vector<fastjet::PseudoJet> testAxes =
                _onePassFinder.getAxes(n_jets, inputJets, noiseAxes);
            double testTau =
                (_measureFunction.result(inputJets, testAxes)).tau();
    ...
    // Uses minimization of the geometric distance in order to find the minimum
    // axes.
    // It continually updates until it reaches convergence or it reaches the
    // maximum number of attempts.
    // This is essentially the same as a stable cone finder.
    std::vector<fastjet::PseudoJet>
    AxesFinderFromGeometricMinimization::getAxes(
        int /*n_jets*/, const std::vector<fastjet::PseudoJet> &particles,
        ...
        for (int i = 0; i < _nAttempts; i++) {
            ...
            std::vector<fastjet::PseudoJet> newAxes(
                seedAxes.size(), fastjet::PseudoJet(0, 0, 0, 0));
            ...
            for (unsigned int i = 0; i < particles.size(); i++) {
                ...
                for (unsigned int j = 0; j < seedAxes.size(); j++) {
                    double tempDist = _function.jet_distance_squared(
                        particles[i], seedAxes[j]);
                    if (tempDist < minDist) {
                        minDist = tempDist;
                        minJ = j;
                    }
            ...
            if (fabs(tempTau - seedTau) < _accuracy)
                break;
            seedTau = tempTau;
        }
