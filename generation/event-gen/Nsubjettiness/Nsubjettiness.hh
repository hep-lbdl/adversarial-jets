...
FASTJET_BEGIN_NAMESPACE // defined in fastjet/internal/base.hh
    ...
    class Nsubjettiness : public FunctionOfPseudoJet<double> {
      ...
      public:
        ...
        Nsubjettiness(int N, Njettiness::AxesMode axes_mode,
                      Njettiness::MeasureMode measure_mode, double para1)
        ...
        Nsubjettiness(int N, Njettiness::AxesMode axes_mode,
                      Njettiness::MeasureMode measure_mode, double para1,
                      double para2)
            : _njettinessFinder(axes_mode, measure_mode, 2, para1, para2),
              _N(N) {
        }

        // Three parameter arguments
        // (for unnormalized_cutoff_measure, para1=beta, para2=R0,
        // para3=Rcutoff)
        Nsubjettiness(int N, Njettiness::AxesMode axes_mode,
                      Njettiness::MeasureMode measure_mode, double para1,
                      double para2, double para3)
            : _njettinessFinder(axes_mode, measure_mode, 3, para1, para2,
                                para3),
              _N(N) {
        }
        ...
        Nsubjettiness(int N, Njettiness::AxesMode axes_mode, double beta,
                      double R0,
                      double Rcutoff = std::numeric_limits<double>::max())
            : _njettinessFinder(axes_mode,
                                NormalizedCutoffMeasure(beta, R0, Rcutoff)),
              _N(N) {
        }
        ...
        /// returns components of tau_N, so that user can find individual tau
        /// values.
        ...
        /// returns seed axes used for onepass minimization (otherwise same as
        /// currentAxes)
        std::vector<fastjet::PseudoJet> seedAxes() const {
            return _njettinessFinder.seedAxes();
        }

        /// returns subjet regions found by result() calculation (these have
        /// valid constituents)
      ...
      private:
        Njettiness _njettinessFinder; // TODO:  should muck with this so result
                                      // can be const without this mutable
        int _N;
    };

    //------------------------------------------------------------------------
    /// \class NsubjettinessRatio
    // NsubjettinessRatio uses the results from Nsubjettiness to calculate the
    // ratio
    // tau_N/tau_M, where N and M are specified by the user. The ratio of
    ...
    class NsubjettinessRatio : public FunctionOfPseudoJet<double> {
      public:
        // Main constructor.  Apart from specifying both N and M, the same
        // options as Nsubjettiness
        NsubjettinessRatio(int N, int M, const AxesDefinition &axes_def,
                           const MeasureDefinition &measure_def)
        ...
        NsubjettinessRatio(int N, int M, Njettiness::AxesMode axes_mode,
                           Njettiness::MeasureMode measure_mode)
        ...
        // returns tau_N/tau_M based off the input jet using result function
        // from Nsubjettiness
        double result(const PseudoJet &jet) const;
