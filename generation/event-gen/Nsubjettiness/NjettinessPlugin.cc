...
FASTJET_BEGIN_NAMESPACE // defined in fastjet/internal/base.hh
    ...
    std::string NjettinessPlugin::description() const {
    ...
    void NjettinessPlugin::run_clustering(ClusterSequence & cs) const {
        ...
        // HACK: remove area information from particles (in case this is called
        // by
        // a ClusterSequenceArea.  Will be fixed in a future FastJet release)
        for (unsigned i = 0; i < particles.size(); i++) {
            particles[i].set_structure_shared_ptr(
        ...
        std::vector<std::list<int>> partition =
            _njettinessFinder.getPartitionList(particles);

        std::vector<fastjet::PseudoJet> jet_indices_for_extras;

        // output clusterings for each jet
        for (size_t i0 = 0; i0 < partition.size(); ++i0) {
            size_t i = partition.size() - 1 -
                       i0; // reversed order of reading to match axes order
            std::list<int> &indices = partition[i];
            if (indices.size() == 0)
                continue;
            while (indices.size() > 1) {
                int merge_i = indices.back();
                indices.pop_back();
                int merge_j = indices.back();
                indices.pop_back();
                int newIndex;
                double fakeDij = -1.0;

                cs.plugin_record_ij_recombination(merge_i, merge_j, fakeDij,
                                                  newIndex);
            ...
            jet_indices_for_extras.push_back(
                cs.jets()[finalJet]); // Get the four vector for the final jets
                                      // to compare later.
        }
        ...
        NjettinessExtras *extras = new NjettinessExtras(
            _njettinessFinder.currentTauComponents(), jet_indices_for_extras,
            _njettinessFinder.currentAxes());
        cs.plugin_associate_extras(
            std::auto_ptr<ClusterSequence::Extras>(extras));
    }
