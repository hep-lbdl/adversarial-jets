...
FASTJET_BEGIN_NAMESPACE // defined in fastjet/internal/base.hh
    ...
    //------------------------------------------------------------------------
    /// \class Njettiness
    // Njettiness uses AxesFinder and MeasureFunction together in order to find
    // tau_N for the event. The user specifies
    // which AxesFinder and which MeasureFunction to use in the calculation, and
    // then Njettiness returns tau_N for the event.
    // It also can return information about the axes and jets it used in the
    // calculation, as well as information about
    ...
    class Njettiness {
      public:
        ...
        enum AxesMode {
            ...
            onepass_antikt_0p2_axes, // one-pass minimization from antikt-0.2
                                     // starting point
            ...
            min_axes,    // axes that minimize N-subjettiness (100 passes by
                         // default)
            manual_axes, // set your own axes with setAxes()
            onepass_manual_axes // one-pass minimization from manual starting
                                // point
            //  These options are commented out because they have not been fully
            //  tested
            //      wta2_kt_axes,        // Winner Take All (alpha = 2) with kt
            //      wta2_ca_axes,         // Winner Take All (alpha = 2) with CA
            //      onepass_wta2_kt_axes, //one-pass minimization of WTA (alpha
            //      = 2) axes with kt
            //      onepass_wta2_ca_axes, //one-pass minimization of WTA (alpha
            //      = 2) axes with ca
        };
        ...
        // Main constructor that uses AxesMode and MeasureDefinition to specify
        // measure
        // Unlike Nsubjettiness or NjettinessPlugin, the value N is not chosen
        Njettiness(const AxesDefinition &axes_def,
                   const MeasureDefinition &measure_def);

        // Intermediate constructor (needed to enable v1.0.3 backwards
        // compatibility?)
        Njettiness(AxesMode axes_mode, const MeasureDefinition &measure_def);

        // Alternative constructor which takes axes/measure information as enums
        // with measure parameters
        // This version is not recommended
        Njettiness(AxesMode axes_mode, MeasureMode measure_mode, int num_para,
            ...
            : _axes_def(createAxesDef(axes_mode)),
              _measure_def(createMeasureDef(measure_mode, num_para, para1,
                                            para2, para3)) {
            setMeasureFunctionAndAxesFinder(); // call helper function to do the
                                               // hard work
        }
        ...
        // This information is stored in _current_tau_components for later
        // access as well.
        TauComponents getTauComponents(
            unsigned n_jets,
        ...
        // Return seedAxes used if onepass minimization (otherwise, same as
        // currentAxes)
        ...
        // partition inputs by Voronoi (each vector stores indices corresponding
        // to inputJets)
        std::vector<std::list<int>> getPartitionList(
            const std::vector<fastjet::PseudoJet> &inputJets) const;
      ...
      private:
        ...
        SharedPtr<AxesFinder>
            _finishingAxesFinder; // A possible minimization step

        // Information about the current information
        // Defined as mutables, so user should be aware that these change when
        // getTau is called.
        mutable TauComponents _current_tau_components; // automatically set to
                                                       // have components of 0;
                                                       // these values will be
                                                       // set by the getTau
                                                       // function call
        mutable std::vector<fastjet::PseudoJet>
            _currentAxes; // axes found after minimization
        mutable std::vector<fastjet::PseudoJet>
            _seedAxes; // axes used prior to minimization (if applicable)
        mutable std::vector<fastjet::PseudoJet>
            _currentJets;                        // partitioning information
        mutable fastjet::PseudoJet _currentBeam; // return beam, if requested

        // created separate function to set MeasureFunction and AxesFinder in
        // order to keep constructor cleaner.
        ...
        // Convert old style enums into new style MeasureDefinition
        MeasureDefinition *createMeasureDef(MeasureMode measure_mode,
                                            int num_para, double para1,
                                            double para2, double para3) const;
    };
