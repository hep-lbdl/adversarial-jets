...
FASTJET_BEGIN_NAMESPACE // defined in fastjet/internal/base.hh
    ...
    // Eventually this file might contains an NjettinessDefinition that combines
    // an
    // AxesDefinition with a MeasureDefinition. It's not clear how useful that
    // would be, though
    ...
    // Below are just technical implementations of the variable axes and
    // measures.
    ...
    // parameters used by Njettiness, with errors in the number of parameters
    // given at
    // compile time (instead of at run time).  The MeasureDefintion knows which
    // core objects
    // to call to make the measurement, as well as properties of the
    // MeasureFunction
    class MeasureDefinition {
      ...
      public:
        ...
        // Specify whether multi-pass minimization makes sense, and if so,
        // return the AxesFinder that should be used for multi-pass minimization
    ...
    class NormalizedMeasure : public MeasureDefinition {
      ...
      public:
        NormalizedMeasure(double beta, double R0) : _beta(beta), _R0(R0) {
        }

        virtual std::string description() const;

        virtual MeasureFunction *createMeasureFunction() const {
            return (new DefaultNormalizedMeasureFunction(
                _beta, _R0, std::numeric_limits<double>::max()));
        }
        ...
        virtual AxesFinder *createOnePassAxesFinder() const {
            return (new AxesFinderFromOnePassMinimization(
                _beta, std::numeric_limits<double>::max()));
        }
        virtual AxesFinder *
        createMultiPassAxesFinder(unsigned int Npass) const {
            return (new AxesFinderFromKmeansMinimization(
                _beta, std::numeric_limits<double>::max(), Npass));
        }

        virtual bool supportsMultiPassMinimization() const {
            return true;
        }
    ...
    class UnnormalizedMeasure : public MeasureDefinition {
      ...
      public:
        UnnormalizedMeasure(double beta) : _beta(beta) {
        }
        ...
        virtual MeasureFunction *createMeasureFunction() const {
            return (new DefaultUnnormalizedMeasureFunction(
                _beta, std::numeric_limits<double>::max()));
        }
    ...
    // This measure is still evolving and shouldn't be used for any critial
    // applications yet
    class GeometricMeasure : public MeasureDefinition {

      public:
        GeometricMeasure(double beta) : _beta(beta) {
        }

        virtual GeometricMeasure *create() const {
            return new GeometricMeasure(*this);
        }

        virtual std::string description() const;

        virtual MeasureFunction *createMeasureFunction() const {
            return (new GeometricMeasureFunction(
                _beta, std::numeric_limits<double>::max()));
        }

        virtual AxesFinder *createOnePassAxesFinder() const {
            return (new AxesFinderFromGeometricMinimization(
                _beta, std::numeric_limits<double>::max()));
        }
        virtual AxesFinder *createMultiPassAxesFinder(unsigned int) const {
            throw Error(
                "GeometricMeasure does not support multi-pass minimization.");
            return NULL;
        }
    ...
    class GeometricCutoffMeasure : public MeasureDefinition {
      ...
      public:
        ...
        virtual GeometricCutoffMeasure *create() const {
            return new GeometricCutoffMeasure(*this);
        }
        ...
        virtual AxesFinder *createMultiPassAxesFinder(unsigned int) const {
            throw Error("GeometricCutoffMeasure does not support multi-pass "
                        "minimization.");
            return NULL;
        }
    ...
    // Analogous to MeasureDefinition, AxesDefinition defines which AxesFinder
    // to use
    // At the moment, most AxesDefinition do not have an arugment (except the
    // Anti-KT ones)
    class AxesDefinition {
      ...
      public:
        ...
        // for best testing
        virtual AxesFinder *
        createStartingAxesFinder(const MeasureDefinition &) const = 0;
        virtual AxesFinder *
        createFinishingAxesFinder(const MeasureDefinition &) const {
    ...
    class KT_Axes : public AxesDefinition {
      public:
        ...
        virtual std::string description() const {
            ...
            stream << std::fixed << std::setprecision(2) << "KT Axes";
            return stream.str();
        };

        virtual KT_Axes *create() const {
            return new KT_Axes(*this);
        }

        virtual bool givesRandomizedResults() const {
            return false;
        }
        virtual bool supportsManualAxes() const {
            return false;
        }

        virtual AxesFinder *
        createStartingAxesFinder(const MeasureDefinition &) const {
    ...
    class CA_Axes : public AxesDefinition {
      public:
        ...
        virtual std::string description() const {
            std::stringstream stream;
            stream << std::fixed << std::setprecision(2) << "CA Axes";
            return stream.str();
        };

        virtual CA_Axes *create() const {
            return new CA_Axes(*this);
        }
    ...
    class AntiKT_Axes : public AxesDefinition {
      ...
      public:
        ...
        virtual std::string short_description() const {
            std::stringstream stream;
            stream << std::fixed << std::setprecision(2) << "AKT" << _R0;
            return stream.str();
        };
    ...
    class OnePass_KT_Axes : public AxesDefinition {
      public:
        ...
        virtual OnePass_KT_Axes *create() const {
            return new OnePass_KT_Axes(*this);
        }
        ...
        virtual AxesFinder *
        createFinishingAxesFinder(const MeasureDefinition &measure_def) const {
    ...
    class OnePass_CA_Axes : public AxesDefinition {
      public:
        ...
        virtual OnePass_CA_Axes *create() const {
            return new OnePass_CA_Axes(*this);
        }
    ...
    class OnePass_AntiKT_Axes : public AxesDefinition {
      ...
      public:
        ...
        virtual std::string short_description() const {
            std::stringstream stream;
            stream << std::fixed << std::setprecision(2) << "OnePassAKT" << _R0;
            return stream.str();
        };
        ...
        virtual OnePass_AntiKT_Axes *create() const {
            return new OnePass_AntiKT_Axes(*this);
        }
    ...
    class OnePass_WTA_KT_Axes : public AxesDefinition {
      public:
        ...
        virtual OnePass_WTA_KT_Axes *create() const {
            return new OnePass_WTA_KT_Axes(*this);
        }
    ...
    class OnePass_WTA_CA_Axes : public AxesDefinition {
      public:
        ...
        virtual OnePass_WTA_CA_Axes *create() const {
            return new OnePass_WTA_CA_Axes(*this);
        }
    ...
    class Manual_Axes : public AxesDefinition {
      public:
        ...
        virtual std::string description() const {
            std::stringstream stream;
            stream << std::fixed << std::setprecision(2) << "Manual Axes";
            return stream.str();
        };

        virtual Manual_Axes *create() const {
            return new Manual_Axes(*this);
        }
    ...
    class OnePass_Manual_Axes : public AxesDefinition {
      public:
        ...
        virtual OnePass_Manual_Axes *create() const {
            return new OnePass_Manual_Axes(*this);
        }
    ...
    class MultiPass_Axes : public AxesDefinition {
      ...
      public:
        ...
        virtual MultiPass_Axes *create() const {
            return new MultiPass_Axes(*this);
        }
        ...
        virtual AxesFinder *
        createFnishingAxesFinder(const MeasureDefinition &measure_def) const {
