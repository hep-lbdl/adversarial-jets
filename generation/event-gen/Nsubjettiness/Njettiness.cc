...
FASTJET_BEGIN_NAMESPACE // defined in fastjet/internal/base.hh
    ...
    Njettiness::Njettiness(const AxesDefinition &axes_def,
    ...
    Njettiness::Njettiness(AxesMode axes_mode,
                           const MeasureDefinition &measure_def)
    ...
    MeasureDefinition *Njettiness::createMeasureDef(
        MeasureMode measure_mode, int num_para, double para1, double para2,
        double para3) const {

        // definition of maximum Rcutoff for non-cutoff measures, changed later
        // by other measures
        ...
        case normalized_measure:
            ...
            if (num_para == 2) {
                ...
                throw Error(
                    "normalized_measure needs 2 parameters (beta and R0)");
            }
            break;
        ...
        case normalized_cutoff_measure:
            beta = para1;
            R0 = para2;
            Rcutoff = para3; // Rcutoff parameter is 3rd parameter in
                             // normalized_cutoff_measure
            if (num_para == 3) {
                return new NormalizedCutoffMeasure(beta, R0, Rcutoff);
            } else {
                throw Error("normalized_cutoff_measure has 3 parameters (beta, "
                            "R0, Rcutoff)");
            }
            break;
        case unnormalized_cutoff_measure:
            beta = para1;
            Rcutoff = para2; // Rcutoff parameter is 2nd parameter in
                             // normalized_cutoff_measure
            if (num_para == 2) {
                return new UnnormalizedCutoffMeasure(beta, Rcutoff);
            } else {
                throw Error("unnormalized_cutoff_measure has 2 parameters "
                            "(beta, Rcutoff)");
            }
            break;
        case geometric_cutoff_measure:
            beta = para1;
            Rcutoff = para2; // Rcutoff parameter is 2nd parameter in
                             // geometric_cutoff_measure
            if (num_para == 2) {
                return new GeometricCutoffMeasure(beta, Rcutoff);
            } else {
                throw Error("geometric_cutoff_measure has 2 parameters (beta, "
                            "Rcutoff)");
            }
            break;
        default:
    ...
    // This returns a pointer that will be claimed by a SharedPtr
    AxesDefinition *Njettiness::createAxesDef(Njettiness::AxesMode axes_mode)
        const {
    ...
    // All of the parameter handling is here, and checking that number of
    ...
    void Njettiness::setMeasureFunctionAndAxesFinder() {
        ...
        _startingAxesFinder.reset(
            _axes_def->createStartingAxesFinder(*_measure_def));
        _finishingAxesFinder.reset(
            _axes_def->createFinishingAxesFinder(*_measure_def));
    }
    ...
    // This information is stored in _current_tau_components for later access as
    // well.
    TauComponents Njettiness::getTauComponents(
        unsigned n_jets, const std::vector<fastjet::PseudoJet> &inputJets)
        const {
        if (inputJets.size() <= n_jets) { // if not enough particles, return
                                          // zero
            ...
            _seedAxes = _startingAxesFinder->getAxes(
                n_jets, inputJets,
                _currentAxes); // sets starting point for minimization
            if (_finishingAxesFinder) {
                _currentAxes =
                    _finishingAxesFinder->getAxes(n_jets, inputJets, _seedAxes);
            } else {
            ...
            _currentJets = _measureFunction->get_partition(
                inputJets, _currentAxes, &_currentBeam);

            // Find tau value and store information
            _current_tau_components = _measureFunction->result_from_partition(
                _currentJets, _currentAxes,
                &_currentBeam); // sets current Tau Values
        }
        return _current_tau_components;
    }

    // Partition a list of particles according to which N-jettiness axis they
    // are closest to.
    ...
    std::vector<std::list<int>> Njettiness::getPartitionList(
