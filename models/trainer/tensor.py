...
# W = np.random.uniform(0, 1, (nb_kernels, input_dim, kernel_dim))


# activation = np.reshape(np.dot(x, W), (-1, nb_kernels, kernel_dim))


# diffs = np.expand_dims(activation, 3) - \
#     np.expand_dims(np.transpose(activation, [1, 2, 0]), 0)
# abs_diffs = np.sum(np.abs(diffs), axis=2)
# minibatch_features = np.sum(np.exp(-abs_diffs), axis=2)
...
class DenseTensor(Layer):
    ...
    def build(self, input_shape):
        ...
        self.W = self.add_weight(((self.first_dim, input_dim, self.last_dim)),
                                 initializer=self.init,
                                 name='{}_W'.format(self.name),
                                 regularizer=self.W_regularizer,
                                 constraint=self.W_constraint)
        if self.bias:
            self.b = self.add_weight((self.first_dim, self.last_dim),
                                     initializer='zero',
